...
class ProfessorLoginTest extends TestCase
{
    ...
    public function professor_cannot_login_with_wrong_password(): void
    {
        ...
        $this->assertGreaterThan(0, DB::table('login_attempts')->where('prof_id', (string)$prof->Prof_ID)->count());
    }
    ...
    public function professor_cannot_login_with_invalid_professor_id(): void
    {
        ...
        $bag = session('errors');
        $flat = $bag ? $bag->all() : [];
        $this->assertTrue(in_array('Professor ID does not exist.', $flat), 'Expected specific missing ID message');
        $this->assertGuest('professor');
    }
    ...
    public function inactive_professor_account_cannot_login(): void
    {
        ...
        $response = $this->from(route('login.professor'))
            ->post('/login-professor', ['Prof_ID' => $prof->Prof_ID, 'Password' => 'secret123']);
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207773, 'Password' => Hash::make('edgepass')]);
        $response = $this->post('/login-professor', [
            'Prof_ID' => $prof->Prof_ID,
            'Password' => ' edgepass ',
        ]);
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207774, 'Password' => Hash::make('validpass')]);
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207776, 'Password' => Hash::make('@G$w0rd!#2024')]);
        $response = $this->post('/login-professor', ['Prof_ID' => 20207776, 'Password' => '@G$w0rd!#2024']);
        $response->assertRedirect(route('dashboard.professor'));
        $this->assertAuthenticatedAs($prof, 'professor');
    }
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207111, 'Password' => Hash::make('ratelimit')]);
        for ($i = 0; $i < 5; $i++) {
            $this->post('/login-professor', ['Prof_ID' => 20207111, 'Password' => 'wrong-'.$i]);
        }
        $resp = $this->post('/login-professor', ['Prof_ID' => 20207111, 'Password' => 'wrong-final']);
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207767, 'Email' => 'remember.prof@example.com', 'Password' => Hash::make('remember123')]);
        $response = $this->post('/login-professor', [
            'Prof_ID' => $prof->Prof_ID,
    ...
    public function professor_remains_authenticated_after_failed_relogin_attempt(): void
    {
        ...
        $this->assertAuthenticatedAs($prof, 'professor');
        $res = $this->post('/login-professor', ['Prof_ID' => 20207778, 'Password' => 'wrong-after']);
        $this->assertAuthenticatedAs($prof, 'professor');
    }
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207133, 'Password' => Hash::make('auditpw')]);
        $this->post('/login-professor', ['Prof_ID' => 20207133, 'Password' => 'auditpw']);
        $row = DB::table('login_attempts')->where('prof_id', (string)$prof->Prof_ID)->orderByDesc('id')->first();
    ...
    {
        $prof = $this->createProfessor(['Prof_ID' => 20207144, 'Password' => Hash::make('specpass123')]);
        ...
        $bag = session('errors');
        $flat = $bag ? $bag->all() : [];
        $this->assertTrue(in_array('Incorrect password.', $flat), 'Expected specific incorrect password message');
    }
}
