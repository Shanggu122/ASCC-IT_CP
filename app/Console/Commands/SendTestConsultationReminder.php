...
class SendTestConsultationReminder extends Command
{
    ...
    $sendAll = (bool)$this->option('all');
        ...
        if ($bookingId && $sendAll) {
            $this->warn('Both bookingId and --all supplied. Ignoring bookingId and sending all for today.');
        }
        ...
        if ($bookings->filter()->isEmpty()) {
            $this->error('No matching booking'.($sendAll ? 's' : '').' found '.($bookingId && !$sendAll ? 'for ID '.$bookingId : 'for today').' (statuses: pending/approved/rescheduled).');
        ...
        foreach ($bookings as $booking) {
            ...
            if (!$prof || !$prof->Email) {
                $this->warn('Skipping booking '.$booking->Booking_ID.' (missing professor email).');
                $failed++; continue;
            }
            ...
            try {
                ...
                Log::error('[TestReminderEmail] Failed', ['booking_id'=>$booking->Booking_ID,'error' => $e->getMessage()]);
                $this->error('Send failed for booking '.$booking->Booking_ID.': '.$e->getMessage());
                $failed++;
            }
        }

        $this->line("Summary: sent=$sent failed=$failed");
        $this->line('Use --all to send all for today, or provide a specific bookingId. Use --to=you@example.com to override destination.');
