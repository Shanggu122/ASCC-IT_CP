...
class PreviewUpcomingConsultationReminder extends Command
{
     ...
     */
    protected $signature = 'consultations:preview-upcoming '
    ...
    public function handle(): int
    {
        ...
        if ($realId = $this->option('use-booking')) {
            $row = DB::table('t_consultation_bookings as b')
                ...
                ->select('b.Booking_ID','b.Booking_Date','b.Prof_ID','s.Name as student_name','subj.Subject_Name as subject_name','p.Name as professor_name')
            ...
            $this->info('Loaded real booking '.$bookingId.' (Prof '.$profId.') for preview.');
        }

        $mailable = new UpcomingConsultationReminder($student,$subjectName,$typeName,$bookingDate,$bookingId,$profId,$profName);

        if (!$to && !$dump && !$sendAndDump) {
            $this->error('Provide --to=address or use --dump to just preview. Tip: use --use-booking=ID for valid action links.');
            return Command::FAILURE;
        }
        ...
        if ($to) {
            if (!preg_match('/@gmail\.com$/i', $to)) {
                $this->warn('Non-Gmail address detected ('.$to.'). Email will not be sent. Use a @gmail.com address or --dump for preview.');
            } else {
            try {
    ...
    $this->line('Done. Customize content with options: --student --subject --type --date --booking --prof --profName OR use --use-booking=ID for real links.');
