...
class CardItis extends Controller
{
    public function showItis()
    {
        ...
        $professors = Professor::where("Dept_ID", 1)->with("subjects")->get();
        ...
        // so the calendar can render instantly without waiting for API calls.
        ...
        if (!empty($profIds)) {
            ...
            // Initialize maps for each professor
            ...
            foreach ($rows as $ov) {
                ...
                foreach ($period as $d) {
                    $iso = $d->format("Y-m-d");
                    // Determine which professor ids this row applies to
                    $targetProfIds = [];
                    ...
                    foreach ($targetProfIds as $pid) {
                        ...
                        // Label mirrors API behavior (UI also derives some labels from reason/effect)
                        $label = null;
        ...
        return view("itis", compact("professors", "consultationTypes", "preloadedOverrides"));
    }
}
